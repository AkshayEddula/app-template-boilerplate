...
import Animated, { Easing, FadeInDown, FadeInUp, LinearTransition } from "react-native-reanimated";
...
export const SquircleCard = ({ item, index, onPress, isCompleted, currentFilter }: any) => {
    const isAvailableToday = isTaskToday(item);

    // Determine base color
    const baseColor = isAvailableToday ? CARD_COLORS[index % CARD_COLORS.length] : INACTIVE_COLOR;
    const textColor = isAvailableToday ? '#1E293B' : '#64748B';

    return (
        <Animated.View
            key={`${item._id}-${currentFilter}`}
            entering={FadeInDown.delay(index * 60).duration(DURATION_SLOW).easing(SMOOTH_EASING)}
            ...
            >
                ...
                */}
                <View style={{
                    ...
                    backgroundColor: Platform.OS === 'android' ? baseColor : 'transparent',
                    ...
                    <View style={{ paddingVertical: 20, paddingHorizontal: 22, flex: 1 }}>
                        ...
                        <View className="flex-row justify-between items-start mb-2">
                            ...
                            <View className="flex-row items-center gap-2">
                                <View className="bg-white/40 px-3 py-1.5 rounded-full flex-row items-center gap-1.5 border border-white/20">
                                    <Ionicons name={isAvailableToday ? "calendar" : "moon"} size={11} color="rgba(0,0,0,0.6)" />
                                ...
                                {isCompleted && isAvailableToday && (
                                    <View className="bg-white px-2.5 py-1.5 rounded-full flex-row items-center shadow-sm">
                                        ...
                                        <Text className="text-emerald-600 text-[10px] font-generalsans-bold ml-1">DONE</Text>
                                    </View>
                                )}
                            </View>
                        </View>
                        ...
                        <View className="flex-row items-center border-t border-black/5 pt-3 mt-auto">
                            ...
                            <Text className="text-slate-500 font-generalsans-medium text-xs">
                                {!isAvailableToday ? "Rest Day" : `Goal: ${getTargetText(item)}`}
                            </Text>
        ...
        <View className="flex-row items-center space-x-1 mt-3">
            {daysMap.map((jsDayIndex, loopIndex) => {
                ...
                return (
                    ...
                    >
                        <Text style={{ fontSize: 9, color: active ? '#FFFFFF' : 'rgba(0,0,0,0.4)', fontWeight: '700' }}>
                            {dayLabels[loopIndex]}
                        </Text>
                    </Animated.View>
                );
